...
export default function NotesPage() {
  ...
  useEffect(() => {
    ...
    if (!uid) return;
    const qN = query(collection(db, 'notes'), where('studentId', '==', uid));
    const unsub = onSnapshot(
      qN,
      (snap) => { setNotes(snap.docs.map(d => ({ id: d.id, ...d.data() }))); setLoadError(false); setLoaded(true); },
      (err) => { console.warn('Notlar yüklenemedi:', err); setLoadError(true); setLoaded(true); }
    );
    return () => unsub();
  }, [reloadKey]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        await addDoc(collection(db, 'notes'), { ...payload, createdAt: safeServerTimestamp() });
