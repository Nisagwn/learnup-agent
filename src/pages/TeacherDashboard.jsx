...
import { doc, getDoc, collection, query, where, onSnapshot, addDoc, updateDoc, deleteDoc, arrayUnion, arrayRemove } from 'firebase/firestore';
...
Yanıtını Türkçe, samimi ve profesyonel bir dille yaz. Maddeler net, uygulanabilir ve motive edici olsun.`;
      ...
      const BACKEND_BASE = import.meta.env.VITE_BACKEND_BASE_URL || (import.meta.env.DEV ? `http://127.0.0.1:5001/${FIREBASE_PROJECT_ID}/us-central1` : `https://us-central1-${FIREBASE_PROJECT_ID}.cloudfunctions.net`);
...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    // Students list with live updates
    const qStudents = query(collection(db, 'users'), where('teacherId', '==', user.uid));
    ...
    // Live updates for user_logs (needed for statistics)
    // Sadece bu öğretmenin sınıfının logları + alan normalizasyonu
    // (user_logs şeması `studentId` kullanır; tüketici kod `user_id` bekler)
    const qAnswers = query(collection(db, 'user_logs'), where('teacherId', '==', user.uid));
    const uAnswers = onSnapshot(qAnswers, (snap) => {
      setUserAnswers(snap.docs.map(d => {
        const x = d.data();
        return {
          id: d.id,
          user_id: x.studentId,
          isCorrect: x.isCorrect === true,
          duration: x.timeSpent ?? 0,
    ...
    // Live updates for assignments & their submissions (öğretmenin kendi sınıfı)
    const qAssign = query(collection(db, 'assignments'), where('teacherId', '==', user.uid));
    const u3 = onSnapshot(qAssign, (snap) => {
      setAssignments(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });
    const qSubs = query(collection(db, 'assignment_submissions'), where('teacherId', '==', user.uid));
    const u4 = onSnapshot(qSubs, (snap) => {
      setSubmissions(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    return () => {
      try { u1(); } catch (err) { console.warn('Student listener cleanup failed:', err); }
      try { uAnswers(); } catch (err) { console.warn('Answers listener cleanup failed:', err); }
      try { u3(); } catch (err) { console.warn('Assignment listener cleanup failed:', err); }
      try { u4(); } catch (err) { console.warn('Submission listener cleanup failed:', err); }
    };
  }, []);
  ...
  const handleRegenerateClassCode = async () => {
    ...
    try {
      const code = await regenerateClassCode(user.uid);
      setClassCode(code);
  ...
  const handleApproveStudent = async (studentId) => {
    try {
      const user = auth.currentUser;
      if (!user) return;
      await updateDoc(doc(db, "users", studentId), { isApproved: true });
      await updateDoc(doc(db, "users", user.uid), { students: arrayUnion(studentId) });
  ...
  const handleRemoveStudent = async (studentId) => {
    try {
      const user = auth.currentUser;
      if (!user) return;
      await updateDoc(doc(db, "users", studentId), { teacherId: null, isApproved: false });
      await updateDoc(doc(db, "users", user.uid), { students: arrayRemove(studentId) });
  ...
  return (
    <div className="dashboard animate-fade-in relative pb-20">
      ...
      <CreateAnnouncementModal isOpen={isAnnounceModalOpen} onClose={()=>setIsAnnounceModalOpen(false)} user={auth.currentUser} classCode={classCode} />
