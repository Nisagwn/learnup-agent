...
export default function LeaguePage() {
  ...
  // Birden çok onSnapshot çağrısı yarışmasın diye istek-id'si
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      async (snap) => {
        ...
        // Sadece öğrencileri tut — eski entry'lerde role yoksa users/{uid}'den oku
        ...
        const filtered = (await Promise.all(
          raw.map(async (e) => {
            ...
            try {
              const u = await getDoc(doc(db, 'users', e.uid));
              const role = u.exists() ? (u.data().role || 'student') : 'student';
        ...
        // Eski snapshot süresi dolmuşsa yaz dökümanı yazma
        if (myReq !== reqIdRef.current) return;
        filtered.sort((a, b) => (b.weeklyXP || 0) - (a.weeklyXP || 0));
        setEntries(filtered);
        setError(false);
      },
      (err) => { console.warn('Lig yüklenemedi:', err); setError(true); },
    );
    return () => unsub();
