...
export default function Chatbot() {
        ...
        try {
          ...
          if (!snap.empty) {
            const latestChatDoc = [...snap.docs].sort((a, b) =>
              getTimestampMs(b.data().lastMessageAt) - getTimestampMs(a.data().lastMessageAt)
            )[0];
            const loadedMessages = (latestChatDoc.data().messages || []);
  ...
  const handleSelectChat = async (chatId) => {
    ...
    if (!u) return;
    try {
      const s = await getDoc(doc(db, `users/${u}/chats`, chatId));
