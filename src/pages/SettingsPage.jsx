...
export default function SettingsPage() {
  ...
  const selectAvatar = async (id) => {
    ...
    try {
      await updateDoc(doc(db, 'users', uid), { avatar: id });
  ...
  const fetchProfile = async () => {
    ...
    const baseline = { uid: user.uid, email: user.email, displayName: user.displayName || null };
    try {
      ...
      if (snap.exists()) {
        ...
        if (data.role === 'student' && data.teacherId) {
          try {
            ...
            if (tSnap.exists()) {
              const t = tSnap.data();
              teacherName = t.name || t.fullName || 'Bilinmeyen Öğretmen';
              fetchedClassCode = t.classCode || '';
            }
  ...
  const handleLogout = async () => {
    try { await signOut(auth); navigate('/'); } catch (err) { console.error('Çıkış hatası:', err); }
  };
