...
// ============================================================
// generateQuestions Cloud Function'ı çağırır (DEPLOYED). Üretilen sorular
// HAVUZA YAZILMAZ (persist YOK); sayfaya sessionStorage ile taşınır.
// Çağrı biçimi mevcut akışla aynı: POST {BASE}/generateQuestions,
// Authorization: Bearer <idToken>, body { ..., userId }.
// ============================================================
...
async function callGenerateEndpoint(payload) {
  ...
  if (!res.ok) {
    ...
    throw new Error(data.error || data.message || `Soru üretilemedi (${res.status})`);
  }
  return res.json();
}
