...
import { signIn, signUp, authErrorMessage } from '../services/authApi';
import {
...
const Auth = () => {
  ...
  /* ── Supabase handler ─────────────────────────────────── */
  // Profil satırı (profiles) auth.users trigger'ıyla otomatik oluşur;
  // signUp options.data → raw_user_meta_data → handle_new_user() trigger.
  const handleAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        await signUp({ email, password, name, role, grade, studentClass });
        toast.success('Kayıt Başarılı!', 'Aramıza hoş geldin 🚀');
        navigate('/dashboard');
      }
    } catch (err) {
      toast.error('Hata', authErrorMessage(err));
    } finally {
      setLoading(false);
    }
  };
