...
export default function Quiz() {
  ...
  const fetchAdaptiveQuestion = useCallback(async (isCorrect = null, givenAnswer = null, duration = null) => {
    ...
    try {
      ...
      if (fnErr) {
        throw new Error(fnErr.message || 'Sunucu hatası');
      }
      ...
      while (idx < ids.length) {
        ...
        try {
          ...
          if (data) {
            ...
            setCurrentQuestion(normalizeQuestion(data));
    ...
    const initData = async () => {
      ...
      if (uid) {
        try {
          ...
          console.error('Failed loading user data in Quiz.initData for uid:', uid, err);
        }
      } else {
        console.warn('Auth currentUser yok / uid alınamadı Quiz.initData');
      }
      // Ödev modunda ise ilgili ödev satırını yükle (hedef soru sayisi icin)
      if (assignmentId) {
        try {
          const { data: aData } = await supabase.from('assignments').select('*').eq('id', assignmentId).single();
          if (aData) {
            // camelCase alias'lar — assignmentRef ve UI questionCount/teacherId okur
            const mapped = { ...aData, teacherId: aData.teacher_id, questionCount: aData.question_count };
            assignmentRef.current = mapped;
  ...
  // Soru değişince favori durumunu Supabase'ten senkronize et
  useEffect(() => {
    let cancelled = false;
    ...
    const uid = currentUid();
    if (!qid || !uid) return;
    supabase.from('bookmarks').select('id').eq('student_id', uid).eq('question_id', qid).maybeSingle()
      .then(({ data }) => { if (!cancelled) setIsBookmarked(!!data); })
      .catch(() => {});
  ...
  const finalizeAssignment = async (st) => {
    ...
    try {
      const row = {
        ...
        max_score: st.answered,
        score: st.answered > 0 ? Math.round((st.correct / st.answered) * 100) : 0,
        status: 'completed',
      };
      // Idempotent: (assignment_id, student_id) için varsa güncelle, yoksa ekle.
      const { data: existing } = await supabase
        ...
        .maybeSingle();
      if (existing?.id) {
        await supabase.from('assignment_submissions').update(row).eq('id', existing.id);
      } else {
  ...
  // Mevcut soruyu favorilere ekler/çıkarır (bookmarks: unique(student_id, question_id))
  const handleToggleBookmark = async () => {
    ...
    try {
      if (isBookmarked) {
        await supabase.from('bookmarks').delete().eq('student_id', uid).eq('question_id', String(q.id));
        setIsBookmarked(false);
      ...
      try {
        ...
        if (uid) {
          await supabase.from('user_logs').insert({
            student_id: uid,
            teacher_id: userData?.teacherId ?? userData?.teacher_id ?? null,
            subject,
