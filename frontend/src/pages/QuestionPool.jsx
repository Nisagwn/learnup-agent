...
// Form verisinden kaydedilebilir questions satırı (snake_case sütunlar) üretir
const buildQuestionDoc = (subjectTr, topic, grade, difficulty, options, correctIndex, questionText, explanation, teacherId, isAI, qualityScore) => {
  ...
  if (typeof qualityScore === 'number') docBody.quality_score = qualityScore;
  return docBody;
};

// questions satırını UI sözleşmesine indir (tüketiciler birçok yerde camelCase okur).
const mapQuestionRow = (r) => ({
  ...r,
  teacherId: r.teacher_id,
  subjectTr: r.subject_tr,
...
const QuestionEditorModal = ({ isOpen, onClose, teacherId, editing, defaultSubject = 'Matematik' }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        const { error: err } = await supabase.from('questions').update(docBody).eq('id', editing.id);
        if (err) throw err;
        success('Soru Güncellendi', 'Değişiklikler kaydedildi.');
      } else {
        // created_at DB'de now() ile otomatik dolar.
        const { error: err } = await supabase.from('questions').insert(docBody);
        if (err) throw err;
...
const AIQuestionGeneratorModal = ({ isOpen, onClose, teacherId, defaultSubject = 'Matematik' }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (fnErr) throw new Error(fnErr.message || 'Üretim başarısız.');
...
export default function QuestionPool() {
  ...
  useEffect(() => {
    ...
    // (1) Kendi sorularım (teacher_id == ben).
    const loadOwn = async () => {
      const { data, error: err } = await supabase.from('questions').select('*').eq('teacher_id', teacherId);
      if (!active) return;
      if (err) { console.warn('Kendi sorular yüklenemedi:', err); setLoadError(true); ownReady = true; markLoaded(); return; }
      setOwnQuestions((data || []).map(mapQuestionRow)); setLoadError(false); ownReady = true; markLoaded();
    };
    loadOwn();
    channels.push(supabase.channel(`qp-own-${teacherId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'questions', filter: `teacher_id=eq.${teacherId}` }, loadOwn)
      .subscribe());

    if (branchSubject) {
      const cats = [...new Set([SUBJECT_EN[branchSubject], branchSubject].filter(Boolean))];
      // (2) Branş havuzu (category in [EN, TR]).
      const loadPool = async () => {
        const { data, error: err } = await supabase.from('questions').select('*').in('category', cats).limit(500);
        if (!active) return;
        if (err) { console.warn('Branş havuzu yüklenemedi:', err); poolReady = true; markLoaded(); return; } // havuz hatası bloklamasın
        setPoolQuestions((data || []).map(mapQuestionRow)); poolReady = true; markLoaded();
      };
      loadPool();
      channels.push(supabase.channel(`qp-pool-${teacherId}`)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'questions', filter: `category=in.(${cats.join(',')})` }, loadPool)
        .subscribe());
    } else {
  ...
  const handleApprove = async (id) => {
    try {
      // NOT: approved_by / approved_at sütunları şemada yok → yalnız verified güncellenir.
      const { error: err } = await supabase.from('questions').update({ verified: true }).eq('id', id);
      if (err) throw err;
  ...
  const handleAiTag = async (q) => {
    ...
    try {
      ...
      const { error: err } = await supabase.from('questions').update({ topic, sub_topic: topic }).eq('id', q.id);
      if (err) throw err;
  ...
  const handleApproveAll = async () => {
    ...
    try {
      // NOT: approved_by / approved_at sütunları şemada yok → yalnız verified güncellenir.
      const { error: err } = await supabase.from('questions').update({ verified: true }).in('id', ids);
      if (err) throw err;
