...
function App() {
  ...
  useEffect(() => {
    // onAuthChange INITIAL_SESSION dahil tetiklenir → ayrı bir "başlangıç oturumu"
    // sorgusuna gerek yok. Supabase Auth uid = auth.users.id (uuid).
    const unsubscribe = onAuthChange(async (currentUser) => {
      ...
      if (currentUser && currentUser.id) {
        ...
        try {
          ...
          if (profile) {
            setUserData({ ...authBaseline, ...profile });
          } else {
            setUserData({ ...authBaseline, role: 'student', isFallback: true });
          }
        } catch (err) {
          console.error('Failed fetching profile for uid:', currentUser?.id, err);
          setUserData({ ...authBaseline, role: 'student', isFallback: true });
        }
      } else {
        setUserData(null);
