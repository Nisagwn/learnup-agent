...
export default function AgendaPanel() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!error) {
        setAssignments((data || []).map((r) => ({ ...r, dueDate: r.due_date, questionCount: r.question_count })));
      }
      setLoadedFor(teacherId);
    };
    load();
    const ch = supabase
      .channel(`agenda-assignments:${teacherId}`)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data } = await supabase
        ...
        .select('*')
        .eq('student_id', uid);
      setSubs((data || []).map((r) => ({ ...r, assignmentId: r.assignment_id })));
    };
    load();
    const ch = supabase
      .channel(`agenda-subs:${uid}`)
