...
import { reauthenticate, signOutUser } from '../../services/authApi';
...
export default function DeleteAccountModal({ isOpen, onClose }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // 2) Backend kalıcı silme (Edge Function: auth kullanıcısı + veri)
      await deleteAccount();
      // 3) Oturumu kapat ve girişe yönlendir
      try { await signOutUser(); } catch { /* token zaten geçersiz olabilir */ }
      navigate('/login', { replace: true });
    } catch (e) {
      setMsg(e.message || 'Hesap silinemedi.');
      setBusy(false);
    }
  };
