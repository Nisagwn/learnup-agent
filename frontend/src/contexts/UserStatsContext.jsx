...
import { onAuthChange } from '../services/authApi';
...
// Supabase user_logs satırını, aşağıdaki istatistik fonksiyonlarının beklediği
// Firestore-benzeri şekle çevirir (created_at → Date, is_correct → isCorrect ...).
// Böylece karmaşık hesaplama fonksiyonlarına dokunmadan aynen çalışırlar.
const mapLogRow = (r) => ({
  ...r,
...
export function UserStatsProvider({ children }) {
    ...
    try {
      ...
      const rankData = await Promise.all(
        students.map(async (student) => {
          ...
          const name = student.name || student.email?.split('@')[0] || 'Öğrenci';
          return { uid: student.id, name, correct, total, net };
        })
      );
  ...
  const setupListeners = useCallback((uid) => {
    ...
    // ── Profil: ilk çekim + realtime (profiles) ──
    const applyProfile = (data) => {
      if (!data || !data.id) return;
      setUserProfile(data);
      if (data.teacher_id) loadClassRanking(uid, data.teacher_id);
    ...
    const profileChannel = supabase
      ...
      .subscribe();
    unsubscribeRefs.current.user = () => supabase.removeChannel(profileChannel);

    // ── Cevaplar: user_logs ilk çekim + her değişimde yeniden hesapla ──
    const recomputeFromLogs = async () => {
      const { data, error: qErr } = await supabase
        .from('user_logs').select('*').eq('student_id', uid);
      if (qErr) { console.warn('Answers fetch error:', qErr); setError(qErr.message); setLoading(false); return; }
      const answers = (data || []).map(mapLogRow);
    ...
    const logsChannel = supabase
      .channel(`logs:${uid}`)
      .on('postgres_changes',
        { event: '*', schema: 'public', table: 'user_logs', filter: `student_id=eq.${uid}` },
        () => recomputeFromLogs())
      .subscribe();
    unsubscribeRefs.current.answers = () => supabase.removeChannel(logsChannel);
  }, [calculateStatsFromAnswers, calculateWeeklyAndMonthly, calculateDailyActivity, loadClassRanking]);

  useEffect(() => {
    const unsubAuth = onAuthChange((user) => {
      if (user) {
        // Bileşenler currentUser?.uid okuyor; Supabase kullanıcısında alan .id →
        // uyumluluk için uid alias'ı ekle.
        setCurrentUser({ ...user, uid: user.id });
        setupListeners(user.id);
        // Günlük görev + lig kaydını sunucuda hazırla (sessizce; backend kapalıysa atlanır)
